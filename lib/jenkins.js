...
function Jenkins(runner) {
  ...
  var self = this;
  var fd, currentSuite, PARALLEL_REPORTING, BARE_SUITES;

  if (process.env.PARALLEL_REPORTING != null) {
    PARALLEL_REPORTING = JSON.parse(process.env.PARALLEL_REPORTING);
  }

  if (process.env.BARE_SUITES != null) {
    BARE_SUITES = JSON.parse(process.env.BARE_SUITES);
  }
  ...
  function genSuiteReport() {
    ...
    writeString(' duration="'+(currentSuite.duration/1000)+'"');
    writeString('>\n');
    ...
    currentSuite.tests.forEach(function(test) {
      ...
      writeString(' duration="'+(test.duration/1000)+'"');
      if (test.state == "failed") {
        writeString('>\n');
        writeString('<failure message="');
        if (test.err.message) writeString(test.err.message);
        writeString('">\n');
  ...
  function unifiedDiff(err) {
    ...
    var lines = msg.split('\n').splice(4);
    return lines.map(cleanUp).filter(notBlank).join('\n');
  }

  runner.on('start', function() {
    var path = process.env.JUNIT_REPORT_PATH;
    if ( PARALLEL_REPORTING ) {
      path = path.substr(0, path.length - 4) + '_' + Date.now() +'_' + process.pid + '.xml';
      if (path) fd = fs.openSync(path, 'w');
    } else {
      if (path) fd = fs.openSync(path, 'w');
    }
    if ( !BARE_SUITES ) writeString('<testsuites name="Mocha Tests">\n');
  });

  runner.on('end', function() {
    endSuite();
    if ( !BARE_SUITES ) writeString('</testsuites>\n');
